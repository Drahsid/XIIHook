...
namespace Config {
	void UpdateUserConfig(UserConfig&uConfig)
	{
		...
		if (!config)
		{
			...
			config << "#In game frametime target. [Recommended: A limit that your PC can hit]\n"
				<< "requestedMinFrameTime=90\n\n"
				<< "#Don't want superfast menus? [Recommended: Keep at 60]\n"
				<< "requestedMinFrameTimeMenus=60\n\n"
				<< "#The frametime target when you are not tabbed into the game.\n"
				<< "requestedMinFrameTimeNoFocus=24\n\n"
				<< "#The in-game field of view. [Recommended: User Preference]\n"
				<< "Fov=83\n\n"
				<< "#The in game gamma (brightness). [Recommended: User Preference]\n"
				<< "gamma=45\n\n"
